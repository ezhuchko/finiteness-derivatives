...
theorem NonIncreasing_pieces [DecidableEq α] {r : RE α} (ni_f : NonIncreasing f) :
  ...
  | l ⬝ r  => by
    ...
    | Or.inl ⟨g1,g2,g3⟩ =>
      ...
      exact ⟨a1 ⬝ r,Sim.Trans Rfl (CatCong a2),
             by simp only [pieces, mem_append, mem_map, and_true, exists_eq_right]; exact Or.inl ⟨a1,a3,rfl⟩⟩
  ...
  | l ⋓ r  => by
    ...
    | Or.inl g1 =>
      ...
      exact ⟨a1,Sim.Trans Rfl a2,by simp only [pieces, mem_append]; exact Or.inl a3⟩
    | Or.inr g1 =>
      have ⟨a1,a2,a3⟩ := (NonIncreasing_pieces ni_f (r:=r)) _ g1
      exact ⟨a1,Sim.Trans Rfl a2,by simp only [pieces, mem_append]; exact Or.inr a3⟩
  | l ⋒ r  => by
    simp[pieces] at h
    ...
    exact ⟨a1 ⋒ b1,InterCong a2 b2,
           by simp only [pieces, productWith, mem_map, Prod.exists, pair_mem_product,
                         exists_eq_right_right, exists_eq_right]; exact ⟨a1,b1,⟨a3,b3⟩,rfl⟩⟩
  | ~r     => by
    simp[pieces] at h
    let ⟨r1,hr1,eq⟩ := h
    subst eq
...
theorem plus_bot_l_ni [DecidableEq α] :
  ...
  match r with
  | ε | Pred ψ | ?=_ | ?!_ | ?<=_ | ?<!_ | _* | _⬝_ | _⋒_ | ~_ => h
  | l ⋓ r  =>
    match l with
    | Pred φ => by
      by_cases g : φ = ⊥
...
theorem mult_eps_ni :
  ...
  | l ⬝ r  =>
    ...
    | Pred ψ | ?=_ | ?!_ | ?<=_ | ?<!_ | _* | _⬝_ | _⋒_ | _⋓_ | ~_ => h
...
theorem plus_not_bot_ni [DecidableEq α] :
  ...
  | l ⋓ r =>
    ...
    | ~l =>
      ...
      | ε | ?=_ | ?!_ | ?<=_ | ?<!_ | _* | _⬝_ | _⋒_ | _⋓_ | ~_ => h
    | ε | Pred ψ | ?=_ | ?!_ | ?<=_ | ?<!_ | _* | _⬝_ | _⋒_ | _⋓_ => h
...
theorem eps_in_pieces' {r : RE α} : ε ∈ pieces' r := by
  ...
  | r* | ~r => simp[pieces']; exact eps_in_pieces'
